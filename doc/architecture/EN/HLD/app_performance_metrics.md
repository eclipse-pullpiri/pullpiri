...
# App Performance Metrics for Monitoring Collection / 모니터링 수집을 위한 App 성능 메트릭 정의
...
## Candidate Metric Research / 후보 메트릭 조사
...
### Existing Delivery Path (Node-Level Metrics) / 기존 전달 경로 (Node 수준 메트릭)
...
### Missing Delivery Path (App-Level Metrics) / 누락된 전달 경로 (App 수준 메트릭)
...
## Confirmed Metrics for PolicyManager Triggering / 수집 대상 메트릭 확정

### Node-Level Metrics (already delivered to PolicyManager) / Node 수준 메트릭 (PolicyManager 전달 경로 존재)
...
### App-Level Metrics (delivery path to PolicyManager is missing) / App 수준 메트릭 (PolicyManager 전달 경로 누락)
...
| Item / 항목 | Status / 상태 |
|-------------|---------------|
| Node-level metrics → MonitoringServer / Node 수준 메트릭 → MonitoringServer | ✅ Implemented / 구현 완료 (`SendNodeInfo` gRPC) |
| Node-level metrics → PolicyManager / Node 수준 메트릭 → PolicyManager | ✅ Implemented / 구현 완료 (`ReportNodeMetrics` gRPC) |
| App-level metrics → MonitoringServer / App 수준 메트릭 → MonitoringServer | ✅ Implemented / 구현 완료 (`SendStressMonitoringMetric` gRPC) |
| App-level metrics stored in etcd / App 수준 메트릭 → etcd 저장 | ✅ Implemented / 구현 완료 |
| **App-level metrics → PolicyManager / App 수준 메트릭 → PolicyManager** | ❌ **Not implemented / 미구현 (missing delivery path / 전달 경로 없음)** |
...
- Issue / 이슈: [#518 – Define App Performance Metrics for Monitoring Collection](https://github.com/eclipse-pullpiri/pullpiri/issues/518)
- Discussion / 토론: [#509 – Stateful Cloud-Local Offloading & App Performance-based Reconcile](https://github.com/eclipse-pullpiri/pullpiri/discussions/509)
- Proto definitions / Proto 정의: `src/common/proto/monitoringserver.proto`, `src/common/proto/policymanager.proto`
