...
**문서 번호**: Pullpiri-CLUSTERING-LLD-2025-001  
**버전**: 1.0  
**날짜**: 2025-09-04  
**작성자**: Pullpiri 팀  
...
Pullpiri 프레임워크는 마스터 노드와 서브 노드(워커 노드) 간의 효율적인 클러스터링 메커니즘을 구현하여 분산 환경에서의 모니터링 및 관리를 가능하게 합니다. 본 클러스터링 시스템은 임베디드 환경의 특수성을 고려하여 설계되었으며, 제한된 리소스, 클라우드 연결성, 소규모 클러스터에 최적화되어 있습니다.

### 1.1 목적 및 범위

본 문서는 다음 내용을 포함합니다:
- Pullpiri 클러스터링 아키텍처 상세 설명
...
API Server는 Pullpiri 클러스터에서 마스터 노드의 핵심 컴포넌트로, 클러스터 구성, 노드 관리, 아티팩트 배포 등을 담당합니다.
...
if command -v sha256sum &> /dev/null && [ -n "$CHECKSUM_URL" ]; then
    echo "바이너리 무결성 확인 중..."
    curl -L ${DOWNLOAD_URL}/checksums.txt -o /tmp/pullpiri_checksums.txt
    if ! (cd ${INSTALL_DIR} && sha256sum -c <(grep ${BINARY_NAME}-${BINARY_SUFFIX} /tmp/pullpiri_checksums.txt)); then
...
Pullpiri의 클러스터링 아키텍처는 임베디드 환경에 최적화된 소규모 클러스터를 위해 설계되었습니다:
...
Pullpiri 임베디드 환경과 클라우드 연계를 위한 최적화된 클러스터 토폴로지를 지원합니다:
...
- HLD/base/pullpiri_network(pharos).md - Pullpiri 네트워크 아키텍처
