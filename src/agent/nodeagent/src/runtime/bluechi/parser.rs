            ...
            for model in models.iter() {
                if model.get_name() == model_name {
                    if let Some(volume_name) = mi.get_resources().get_volume() {
                        ...
                        let volume_str: String = common::kvstore::get(&key).await?;
                    ...
                    if let Some(network_name) = mi.get_resources().get_network() {
                        ...
                        let network_str = common::kvstore::get(&key).await?;
...
spec:
  volume:
    - name: data
      emptyDir: {}
"#;
        common::kvstore::put("Volume/test-volume", volume_yaml)
            .await
            .unwrap();
...
spec:
  interfaces:
    - name: eth0
      bridge: br0
"#;
        common::kvstore::put("Network/test-network", network_yaml)
            .await
            .unwrap();
...
"#;
        ...
        common::kvstore::put("Model/test-model", model_yaml)
            .await
            .unwrap();
