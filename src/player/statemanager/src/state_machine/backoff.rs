...
use common::statemanager::{ModelState, ErrorCode};
...
impl BackoffManager {
    pub fn check_backoff_period(
        ...
        if current_state == ModelState::CrashLoopBackOff as i32 {
            if let Some(backoff_time) = backoff_timers.get(resource_key) {
                ...
                debug!("Backoff period elapsed for {}, proceeding with transition", resource_key);
            }
        }
        Ok(())
    }
    ...
    pub fn restore_backoff_timer(
        ...
        let current_state_int = StateUtilities::enum_str_to_int(
            &state.current_state, 
            state.resource_type
        );
